...
**进行前端页面 UI 风格的实现或改动时，必须先查阅 [`docs/SPEC/index.md`](docs/SPEC/index.md) 中的设计规范。**

### 核心原则

1. **设计规范是唯一事实来源**：所有 UI 实现（颜色、动画、交互模式）必须遵循 `docs/SPEC/` 中的定义
2. **修改前先确认**：改动 UI 样式前，先检查规范文档是否有明确指引；如无，则主动询问确认
3. **保持一致性**：新建组件时复用规范中定义的颜色变量、动画参数、交互模式

### 规范文档索引

| 文档 | 内容 |
|------|------|
| `docs/SPEC/design-system.md` | 设计风格、颜色系统、交互动画、下拉菜单规范 |
